...
public class Participant {
	...
	public String preparePublishConnection() {
		log.info("USER {}: Request to publish video in room {} by " + "initiating connection from server", this.name,
				this.room.getName());
	...
	public String publishToRoom(SdpType sdpType, String sdpString, boolean doLoopback,
		...
		log.info("USER {}: Request to publish video in room {} (sdp type {})", this.name, this.room.getName(), sdpType);
	...
	public String receiveMediaFrom(Participant sender, String sdpOffer) {
		...
		log.info("USER {}: Request to receive media from {} in room {}", this.name, senderName, this.room.getName());
		...
		if (senderName.equals(this.name)) {
			...
			throw new RoomException(Code.USER_NOT_STREAMING_ERROR_CODE, "Can loopback only when publishing media");
		}
		...
		try {
			...
			if (subscriber.getEndpoint() == null) {
				throw new RoomException(Code.MEDIA_ENDPOINT_ERROR_CODE, "Unable to create subscriber endpoint");
			}
		...
		try {
			...
			log.info("USER {}: Is now receiving video from {} in room {}", this.name, senderName, this.room.getName());
			return sdpAnswer;
	...
	public void cancelReceivingMedia(String senderName) {
		...
		if (subscriberEndpoint == null || subscriberEndpoint.getEndpoint() == null) {
			...
			log.debug("PARTICIPANT {}: Cancel subscriber endpoint linked to user {}", this.name, senderName);
	...
	public void unmuteSubscribedMedia(Participant sender) {
		...
		if (subscriberEndpoint == null || subscriberEndpoint.getEndpoint() == null) {
			...
			if (subscriberEndpoint.getMuteType() == null) {
				...
				log.debug("PARTICIPANT {}: Unmute subscriber endpoint linked to user {}", this.name, senderName);
	...
	public void close() {
		...
		for (String remoteParticipantName : subscribers.keySet()) {
			...
			if (subscriber != null && subscriber.getEndpoint() != null) {
				...
				log.debug("PARTICIPANT {}: Released subscriber endpoint to {}", this.name, remoteParticipantName);
			} else {
	...
	public SubscriberEndpoint getNewOrExistingSubscriber(String remoteName) {
		...
		SubscriberEndpoint existingSendingEndpoint = this.subscribers.putIfAbsent(remoteName, sendingEndpoint);
		if (existingSendingEndpoint != null) {
			sendingEndpoint = existingSendingEndpoint;
			log.trace("PARTICIPANT {}: Already exists a subscriber endpoint to user {}", this.name, remoteName);
		} else {
	...
	private void releaseSubscriberEndpoint(String senderName, SubscriberEndpoint subscriber) {
		if (subscriber != null) {
			...
			log.warn("PARTICIPANT {}: Trying to release subscriber endpoint for '{}' but is null", name, senderName);
		}
	}
		...
		try {
			element.release(new Continuation<Void>() {
				...
				public void onSuccess(Void result) throws Exception {
					log.debug("PARTICIPANT {}: Released successfully media element #{} for {}", Participant.this.name,
							eid, senderName);
				}

				@Override
				public void onError(Throwable cause) throws Exception {
					log.warn("PARTICIPANT {}: Could not release media element #{} for {}", Participant.this.name, eid,
							senderName, cause);
				}
			});
		} catch (Exception e) {
			log.error("PARTICIPANT {}: Error calling release on elem #{} for {}", name, eid, senderName, e);
		}
	}
